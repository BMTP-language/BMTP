BasicVar opr_sub_const(const BasicVar &x,const BasicVar &y);

vector<signle_opr_t> opr_sub_t;

pointer opr_sub_tlf(const pointer& x,const pointer &y)
{
...
pointer opr_sub_int(const pointer& x,const pointer &y)
{
...
pointer opr_sub_uint(const pointer& x,const pointer &y)
{
...
pointer opr_sub_ll(const pointer& x,const pointer &y)
{
...
pointer opr_sub_ull(const pointer& x,const pointer &y)
{
...
pointer opr_sub_short(const pointer& x,const pointer &y)
{
...
pointer opr_sub_ushort(const pointer& x,const pointer &y)
{
...
pointer opr_sub_float(const pointer& x,const pointer &y)
{
...
pointer opr_sub_double(const pointer& x,const pointer &y)
{
...
pointer opr_sub_ldouble(const pointer& x,const pointer &y)
{
...
pointer opr_sub_vector(const pointer& x,const pointer &y)
{
	...
	{
		z.get<vector<BasicVar>>()[i]=opr_sub_const(x.get<vector<BasicVar>>()[i],y.get<vector<BasicVar>>()[i]);
	}
	return z;
}
...
BasicVar opr_sub_const(const BasicVar &x,const BasicVar &y)
{
	...
	{
		z.first=x.first;
		if(opr_sub_t[x.first]==nullptr)
		{
			oneMessage mes;
			ErrorMessage err;
			mes.fun="BasicVar opr_sub_const(const BasicVar &x,const BasicVar &y)";
			mes.description="particular operator is not found";
			mes.other.push_back("operator-[O"+to_string(Opr_Sub)+"]");
		...
		z.second=opr_sub_t[x.first](x.second,y.second);
		return z;
	}
	if(TypeList[x.first].typetype==TypeList[y.first].typetype)
	{
		if(TypeList[x.first].subtypetype>TypeList[y.first].subtypetype)
		{
			...
			z.second=opr_sub_t[x.first](x.second,ImplicitTypeConversion(y,x.first).second);
			return z;
		}
		else
		{
			z.first=y.first;
			if(opr_sub_t[y.first]==nullptr)
			{
				oneMessage mes;
				ErrorMessage err;
				mes.fun="BasicVar opr_sub_const(const BasicVar &x,const BasicVar &y)";
			...
			z.second=opr_sub_t[y.first](ImplicitTypeConversion(x,y.first).second,y.second);
			return z;
		}
	}
	//xy均为数字
	if(TypeList[x.first].typetype<=4&&TypeList[y.first].typetype<=4)
	{
		...
		{
			z.first=y.first;
			z.second=opr_sub_t[x.first](ImplicitTypeConversion(x,y.first).second,y.second);
			return z;
		}
	}
	if(TypeList[x.first].typetype==TypeType_array&&TypeList[y.first].typetype<=4)
	{
		...
		for(size_t i=0;i<s;i++)
		{
			try
			{
				z.second.get<vector<BasicVar>>()[i]=opr_sub_const(x.second.get<vector<BasicVar>>()[i],y);
			}
...
smart_ptr<BasicVar> opr_sub(smart_ptr<BasicVar> x,smart_ptr<BasicVar> y)
{
	smart_ptr<BasicVar> v;
	v.malloc(opr_sub_const(*x,*y));
	return v;
}