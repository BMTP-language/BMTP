smart_ptr<BasicVar> opr_xor(smart_ptr<BasicVar> x,smart_ptr<BasicVar> y)
{
	if(x->first!=VarType_bool||y->first!=VarType_bool)
	{
		oneMessage mes;
		ErrorMessage err;
		mes.fun="smart_ptr<BasicVar> opr_xor(smart_ptr<BasicVar> x,smart_ptr<BasicVar> y)";
		mes.description="expect a Boolean variable for particular operator";
		mes.other.push_back("operator XOR[O"+to_string(Opr_Xor)+"]");
