BasicVar opr_mul_const(const BasicVar &x,const BasicVar &y);

vector<signle_opr_t> opr_mul_t;

pointer opr_mul_tlf(const pointer& x,const pointer &y)
{
...
pointer opr_mul_int(const pointer& x,const pointer &y)
{
...
pointer opr_mul_uint(const pointer& x,const pointer &y)
{
...
pointer opr_mul_ll(const pointer& x,const pointer &y)
{
...
pointer opr_mul_ull(const pointer& x,const pointer &y)
{
...
pointer opr_mul_short(const pointer& x,const pointer &y)
{
...
pointer opr_mul_ushort(const pointer& x,const pointer &y)
{
...
pointer opr_mul_float(const pointer& x,const pointer &y)
{
...
pointer opr_mul_double(const pointer& x,const pointer &y)
{
...
pointer opr_mul_ldouble(const pointer& x,const pointer &y)
{
...
BasicVar opr_mul_const(const BasicVar &x,const BasicVar &y)
{
	...
	{
		z.first=x.first;
		if(opr_mul_t[x.first]==nullptr)
		{
			oneMessage mes;
			ErrorMessage err;
			mes.fun="BasicVar opr_mul_const(const BasicVar &x,const BasicVar &y)";
			mes.description="particular operator is not found";
			mes.other.push_back("operator*[O"+to_string(Opr_Mul)+"]");
		...
		z.second=opr_mul_t[x.first](x.second,y.second);
		return z;
	}
	if(TypeList[x.first].typetype==TypeList[y.first].typetype)
	{
		if(TypeList[x.first].subtypetype>TypeList[y.first].subtypetype)
		{
			...
			z.second=opr_mul_t[x.first](x.second,ImplicitTypeConversion(y,x.first).second);
			return z;
		}
		else
		{
			z.first=y.first;
			if(opr_mul_t[y.first]==nullptr)
			{
				oneMessage mes;
				ErrorMessage err;
				mes.fun="BasicVar opr_mul_const(const BasicVar &x,const BasicVar &y)";
			...
			z.second=opr_mul_t[y.first](ImplicitTypeConversion(x,y.first).second,y.second);
			return z;
		}
	}
	//xy均为数字
	if(TypeList[x.first].typetype<=4&&TypeList[y.first].typetype<=4)
	{
		...
		{
			z.first=y.first;
			z.second=opr_mul_t[x.first](ImplicitTypeConversion(x,y.first).second,y.second);
			return z;
		}
	}
	if(TypeList[x.first].typetype==TypeType_array&&TypeList[y.first].typetype<=4)
	{
		...
		for(size_t i=0;i<s;i++)
		{
			try
			{
				z.second.get<vector<BasicVar>>()[i]=opr_mul_const(x.second.get<vector<BasicVar>>()[i],y);
			}
...
smart_ptr<BasicVar> opr_mul(smart_ptr<BasicVar> x,smart_ptr<BasicVar> y)
{
	smart_ptr<BasicVar> v;
	v.malloc(opr_mul_const(*x,*y));
	return v;
}